...
function parsearPreguntasWord(textoPlano) {
  ...
  const regexEnunciado = /^\d+[\.\)]\s*(.+)$/;
  const regexOpcion = /^([A-Da-d])[\.\)]\s*(.*)$/;
  ...
  function cerrarPreguntaActual() {
    if (!actual) return;
    const opcionesCompletas = actual.opciones.every((o) => o && o.trim() !== "");
    if (
      actual.enunciado.trim() !== "" &&
      opcionesCompletas &&
      actual.correcta !== null &&
      actual.correcta !== undefined
    ) {
      preguntasImportadas.push({
        id: Date.now() + Math.floor(Math.random() * 100000),
        enunciado: actual.enunciado.trim(),
        opciones: actual.opciones,
        correcta: actual.correcta,
        retroalimentacion: actual.retroalimentacion.trim()
      });
    } else {
      errores.push(`Pregunta ${numeroPregunta} ("${actual.enunciado.slice(0, 40) || "sin enunciado"}...") — faltan datos (enunciado, 4 alternativas o respuesta correcta).`);
    }
  }

  lineas.forEach((linea) => {
    const matchEnunciado = linea.match(regexEnunciado);
    const matchOpcion = linea.match(regexOpcion);
    const matchCorrecta = linea.match(regexCorrecta);
    const matchRetro = linea.match(regexRetro);

    if (matchEnunciado && !matchOpcion) {
      cerrarPreguntaActual();
      numeroPregunta++;
      actual = {
        enunciado: matchEnunciado[1],
        opciones: ["", "", "", ""],
        correcta: null,
        retroalimentacion: ""
      };
    ...
    if (!actual) return; // texto antes de la primera pregunta (instrucciones) — se ignora

    if (matchOpcion) {
      const indice = "abcd".indexOf(matchOpcion[1].toLowerCase());
      if (indice !== -1) actual.opciones[indice] = matchOpcion[2];
      return;
    }

    if (matchCorrecta) {
      actual.correcta = "abcd".indexOf(matchCorrecta[1].toLowerCase());
      return;
    }

    if (matchRetro) {
      actual.retroalimentacion = matchRetro[1];
      return;
    }

    // Línea de continuación: texto envuelto en varias líneas.
    // Si ya se registró texto de retroalimentación, se asume que sigue siendo
    // parte de ella; si aún no hay ninguna alternativa capturada, se asume
    // que es continuación del enunciado. Cualquier otro caso (texto suelto
    // entre secciones) se ignora para no ensuciar los datos.
    if (actual.retroalimentacion !== "") {
      actual.retroalimentacion += " " + linea;
    } else if (actual.opciones.every((o) => o === "")) {
