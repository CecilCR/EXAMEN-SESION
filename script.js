/* =====================================================
   ...
   - Importación Word: se descarta el texto previo a la Pregunta 1
     (título del examen, instrucciones, separadores) cuando esa pregunta
     está numerada manualmente ("1. ..."); antes se colaba dentro del
     enunciado por no existir una pregunta anterior que delimitara el inicio
===================================================== */
...
function guardarJSON() {
  ...
  // Se agrega fecha y hora (America/Lima) al nombre del archivo descargado
  // para poder distinguir versiones y evitar subir a GitHub un archivo
  // viejo por confusión con duplicados en la carpeta de Descargas
  // (ej. "preguntas (1).json"). Para publicar el examen, el archivo debe
  // renombrarse a "preguntas.json" (o al nombre que uses con ?banco=...).
  enlace.download = "preguntas_" + marcaDeTiempo() + ".json";
  enlace.click();
...
function marcaDeTiempo() {
  ...
  return `${obtener("year")}-${obtener("month")}-${obtener("day")}_${obtener("hour")}${obtener("minute")}`;
}
...
function expandirAlternativasConcatenadas(lineas) {
  const resultado = [];
  lineas.forEach((linea) => {
    // Solo se intenta dividir si la línea empieza como una alternativa "A)" / "A."
    if (!/^A[\.\)]\s/.test(linea)) {
      resultado.push(linea);
      return;
    }
    // Debe contener al menos 2 marcadores más (B, C o D) para asumir que son
    // varias alternativas concatenadas en un mismo párrafo.
    const marcadores = linea.match(/\s[B-D][\.\)]\s/g);
    if (!marcadores || marcadores.length < 2) {
      resultado.push(linea);
      return;
    }
    const partes = linea.split(/\s(?=[B-D][\.\)]\s)/).map((p) => p.trim()).filter((p) => p.length > 0);
...
function parsearPreguntasWord(textoPlano) {
  let lineas = textoPlano
    .split("\n")
    .map((l) => l.trim())
    .filter((l) => l.length > 0);

  // Si las 4 alternativas quedaron escritas en un solo párrafo de Word
  // (típico cuando se usó Shift+Enter en vez de Enter entre alternativas),
  // esta línea se separa automáticamente en 4 líneas independientes antes
  // de continuar con el análisis posicional.
  lineas = expandirAlternativasConcatenadas(lineas);
  ...
  idxCorrectas.forEach((idxCorrecta, n) => {
    ...
    if (idxOpciones[0] <= idxCorrectaAnterior) {
      errores.push(
        `Pregunta ${numeroVisible}: no se encontraron 4 alternativas completas justo antes de "Respuesta correcta:".`
      );
      return;
    }

    // El enunciado puede abarcar varios párrafos (casos/escenarios extensos).
    // Toma todo lo que hay entre el final del bloque anterior y el inicio de
    // las alternativas, descartando cualquier línea suelta que sea en
    // realidad texto residual de la retroalimentación de la pregunta previa
    // (ej. "Retroalimentación de distractores: ...").
    let idxInicioBloqueAnterior = idxCorrectaAnterior;
    if (idxCorrectaAnterior !== -1 && idxCorrectaAnterior + 1 < lineas.length && regexRetro.test(lineas[idxCorrectaAnterior + 1])) {
      idxInicioBloqueAnterior = idxCorrectaAnterior + 1;
    }
    const idxInicioEnunciado = idxInicioBloqueAnterior + 1;
    const idxFinEnunciado = idxOpciones[0] - 1;

    let candidatasEnunciado = lineas
      .slice(idxInicioEnunciado, idxFinEnunciado + 1)
      .filter((l) => !/^retroalimentaci[oó]n/i.test(l));

    if (candidatasEnunciado.length === 0) {
      errores.push(`Pregunta ${numeroVisible}: no se encontró el enunciado antes de las 4 alternativas.`);
      return;
    }

    // Si dentro del bloque aparece una línea con numeración manual ("1. ...",
    // "2) ..."), se asume que es el verdadero inicio del enunciado y todo lo
    // anterior (título del examen, instrucciones, separadores) se descarta.
    // Esto es clave para la Pregunta 1, donde no existe una pregunta previa
    // que delimite dónde empieza el texto real.
    const idxNumeroEnBloque = candidatasEnunciado.findIndex((l) => regexNumeroPregunta.test(l));
    if (idxNumeroEnBloque > 0) {
      candidatasEnunciado = candidatasEnunciado.slice(idxNumeroEnBloque);
    }

    const mEnun = candidatasEnunciado[0].match(regexNumeroPregunta);
    if (mEnun) candidatasEnunciado[0] = mEnun[1];
    let enunciado = candidatasEnunciado.join(" ");
